// helpers.js - Versión corregida y validada (resumeAudio, startBreathFlow, touch/click binds, TTS, favoritos, fallbacks)
// Reemplaza el helpers.js actual por este archivo. No traduzcas ni modifiques el código accidentalmente al pegar.
(function () {
  'use strict';
  console.log('[helpers] cargando helpers.js (corregido)');
  ...
  // Valores por defecto (ajustables desde la API)
  ...
  function playHtml(url, offset, duration) {
    ...
    try {
      ...
      try { el.currentTime = offset; } catch (e) { /* some browsers disallow setting */ }
      el.volume = 0.95;
  ...
  async function preloadAssets() {
    ...
    // breath whole file
    let chosen = null;
    for (var i = 0; i < AUDIO.breathCandidates.length; i++) {
      var c = AUDIO.breathCandidates[i];
    ...
    // ambient
    let amb = null;
    for (var j = 0; j < AUDIO.ambientCandidates.length; j++) {
      var a = AUDIO.ambientCandidates[j];
      // eslint-disable-next-line no-await-in-loop
  ...
  function loadVoicesOnce() {
    return new Promise(function (resolve) {
      ...
      var onVoices = function () { window.speechSynthesis.removeEventListener('voiceschanged', onVoices); resolve(speechSynthesis.getVoices() || []); };
  ...
  async function playTTS(text) {
    ...
    var vs = await loadVoicesOnce();
    var voice = (vs.find(function (v) { return /^es/i.test(v.lang); }) || vs[0]) || null;
  ...
  async function startBreathFlowInternal() {
    ...
    async function loopStep() {
      ...
      try {
        var scaleFrom = s.label === 'Exhala' ? 1 : 0.6;
        var scaleTo = s.label === 'Exhala' ? 0.6 : 1.0;
        if (circle.animate) {
  ...
  function toggleFavorite(text) {
    ...
    if (favs.indexOf(text) !== -1) {
      var next = favs.filter(function (x) { return x !== text; });
      saveFavoritos(next);
      return false;
    } else {
      favs.unshift(text);
  ...
  function showFavoritesModal() {
    var favs = getFavoritos();
    var modal = document.getElementById('_lr_fav_modal') || document.createElement('div');
    ...
    var inner = '<div style="display:flex;justify-content:space-between;align-items:center"><strong>Favoritos</strong><button id="_lr_close_fav" style="background:transparent;color:#fff;border:1px solid rgba(255,255,255,0.06);padding:6px;border-radius:8px">Cerrar</button></div><hr style="opacity:.08;margin:8px 0">';
    if (favs && favs.length) {
    ...
    var close = document.getElementById('_lr_close_fav');
    if (close) close.addEventListener('click', function () { modal && modal.parentNode && modal.parentNode.removeChild(modal); });
  }
  function escapeHtml(s) { return (s || '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;'); }

  /* ---------- UI attach helpers (touch + click) ---------- */
  ...
  document.addEventListener('DOMContentLoaded', function () {
    // robust binds a IDs conocidos
    attachTouchClick(['breathBtn_menu', 'breathBtn'], function () { startBreathFlow(); });
    attachTouchClick(['enableAudioBtn', 'enableAudio'], function () { resumeAudio().then(function () { alert('Intentado activar audio'); }); });
    attachTouchClick(['ttsBtn_menu', 'ttsBtn'], function () { var t = (document.getElementById('frase') && document.getElementById('frase').textContent) || ''; if (t) playTTS(t); });
    ...
    attachTouchClick(['favBtn_menu', 'favBtn'], function () { var t = (document.getElementById('frase') && document.getElementById('frase').textContent) || ''; if (t) { var added = toggleFavorite(t); var el = document.getElementById('favBtn_menu') || document.getElementById('favBtn'); if (el) el.textContent = added ? '♥ Favorita' : '♡ Favorita'; } });
    // cerrar menú tras click
    var panel = document.getElementById('menuPanel');
    if (panel) panel.addEventListener('click', function () { setTimeout(function () { panel.style.display = 'none'; var tgl = document.getElementById('menuToggle'); if (tgl) tgl.setAttribute('aria-expanded', 'false'); }, 80); });
  });
  ...
  Object.assign(window.lr_helpers, {
    ...
    setBreathPattern: function (name) { var PRE = { box: { inh: 4, h1: 4, exh: 4, h2: 4 }, calm: { inh: 4, h1: 4, exh: 6, h2: 1 }, slow: { inh: 5, h1: 5, exh: 7, h2: 1 }, '478': { inh: 4, h1: 7, exh: 8, h2: 1 } }; var p = PRE[name]; if (!p) { console.warn('preset not found', name); return; } inhaleDurationSeconds = p.inh; hold1DurationSeconds = p.h1; exhaleDurationSeconds = p.exh; hold2DurationSeconds = p.h2; console.log('[helpers] preset applied', name); },
