...
(function () {
  ...
  async function pickExisting(candidates) {
    for (const c of candidates) {
      if (await existsUrl(c)) {
        console.log('[helpers] encontrado audio disponible ->', c);
        return c;
      }
    }
    return null;
  ...
  async function startBreathFlow() {
    ...
    async function loopStep() {
      ...
      try {
        const scaleFrom = s.label === 'Exhala' ? 1.0 : 0.6;
    ...
    overlay._stop = () => {
      ...
      console.log('[helpers] breath overlay stopped by user');
    };
