...
// Versión con corrección: elimina también la línea
// "Selecciona duración o un preset y pulsa Iniciar."
// Cambios mínimos: añadida regex para esa frase en las limpiezas.
...
(function () {
    ...
    function findSubtitleNode(container) {
        ...
        try {
            ...
            var el = Array.from(container.querySelectorAll('*')).find(function (e) {
            ...
            // fallback general
            var fallback = Array.from(container.querySelectorAll('p,div,span')).find(function (n) {
                var t = (n.textContent || '').trim();
                if (!t || t.length < 8) return false;
                if (n.querySelector && n.querySelector('button,select')) return false;
    ...
    function personalizeContainer(container, name, timerText) {
        ...
        try {
            userData =
                window.CLIENT_USER ||
                JSON.parse(localStorage.getItem('lr_client_runtime') || localStorage.getItem('lr_client_runtime_user') || '{}');
        } catch (e) {
        ...
        var message = buildMessage(name || (userData && userData.nombre) || 'amiga', phrase);

        // Evitar modificaciones repetidas
        var existingName = container.getAttribute && container.getAttribute(NAME_ATTR);
        var existingStored = container.getAttribute && container.getAttribute('data-breath-mood-phrase');
        ...
        try {
            ...
            if (target) {
                ...
                // Eliminar frases antiguas (AQUÍ he añadido la regex para la línea en azul)
                Array.from(container.querySelectorAll('*')).forEach(function (n) {
                    try {
                        var t = (n.textContent || '').trim();
                        if (!t) return;

                        if (
                            /Solo tienes que seleccionar el tiempo que puedes dedicarle|Tras ella, te sentirás mejor|ya tenemos preparada tu sesión|Selecciona duraci[oó]n o un preset y pulsa Iniciar\.?/i.test(
                                t
                            ) &&
                            n !== insert
                        ) {
