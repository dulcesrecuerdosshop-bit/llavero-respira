...
(function(){
  ...
  function mostrarFrase() {
    ...
    setTimeout(()=>{
      ...
      const chosenBg = fondosDisponibles[j] || gradientFondos[j % gradientFondos.length];
  ...
  function wireControls() {
    ...
    function callWhenHelpers(fn){
      if (window.lr_helpers) { try { fn(); } catch(e){ if (window.LR_DEBUG) console.warn('[phrases] helper call failed', e); } return; }
      let waited = 0;
      const iv = setInterval(()=> {
        waited += 100;
        if (window.lr_helpers) {
          clearInterval(iv);
          try{ fn(); } catch(e){ if (window.LR_DEBUG) console.warn('[phrases] helper call failed after wait', e); }
